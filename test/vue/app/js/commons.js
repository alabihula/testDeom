define(['jquery'], function($) {
	var initObj = function() {
		...
		this.getUrl = function(obj) {
			if(obj.url.other) {
				return obj.url.other;
			} else {
				return this.options.baseUrl[this.options.useOn] + obj.url[this.options.useOn];
			}
		}
		...
		this.ajax = function(obj) {
			$.ajax({
				type: obj.urlObj.type || 'GET',
				url: this_.getUrl(obj.urlObj),
				dataType: obj.urlObj.dataType || 'json',
				...
				error: obj.error || function(e) {
					...
					this_.alertMsg(obj.urlObj.name, JSON.stringify(e));
				}
			});
		};
		...
		this.alertMsg = function(name,msg) {
			console.log(name+"-"+msg);
		}

		this.getQueStr = function(url, paras) { //取获参数值  
		...
		this.delQueStr = function(url, ref) { //删除参数值  
			var str = "";
			if(url.indexOf('?') != -1)
